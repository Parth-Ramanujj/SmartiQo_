  ...
  try {
    ...
    if (!item && cartParamId) {
        try {
            ...
            item = localOrders.find(i => i.id === cartParamId || i.productSequence === cartParamId);
        } catch(e) {}
    }
    if (!item && cartParamId) {
        try {
            let dashOrders = JSON.parse(localStorage.getItem('sc_fetched_dashboard_orders') || '[]');
            item = dashOrders.find(i => i.id === cartParamId || i.productSequence === cartParamId);
        } catch(e) {}
    }
    ...
    for (const k of keys) {
      ...
      try {
          ...
          if (parsed.cartData) {
            ...
            const currentItemStr = JSON.stringify(cd.cartItems);
            const newItemStr = JSON.stringify(item.cartData);
            
            if (currentItemStr !== newItemStr) {
                cd.cartItems = item.cartData; // FIXED PROPERTY
